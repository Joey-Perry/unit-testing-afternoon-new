...
test('check if inner text content contains passed in posts text content', () => {
...
test('check if PostWidget shortens text', () => {
    const { container } = render(
        <MemoryRouter>
            <PostWidget {...longPost} />
        </MemoryRouter>
...
test('check is PostWidget displays all text', () => {
    const { container } = render(
        <MemoryRouter>
